                    ...
                    <?php foreach($arrgroup as $v):?>
                        <tr class="gradeX">
                          ...
                          <td><?= Html::encode($v->group_desc) ?></td>
                            <td><?= Html::encode(date("Y-m-d H:i:s",$v->group_addtime)) ?></td>
                            <td><?= Html::encode(date("Y-m-d H:i:s",$v->group_endtime)) ?></td>
                            <td><?= Html::encode(date("Y-m-d H:i:s",$v->group_starttime)) ?></td>
                            <!-- 修改 -->
                            <td>
                            <a href="<?php echo Yii::$app->urlManager->createUrl('app/upgroup')?>&group_id=<?= Html::encode($v->group_id) ?>"><img width="10px" height="10px" src="<?php echo yii::$app->request->baseUrl?>/assets/images/gai.jpg" alt="" /></a>
                            <!-- 删除 -->
                            &nbsp;&nbsp;<a href="<?php echo Yii::$app->urlManager->createUrl('app/delgroup')?>&group_id=<?= Html::encode($v->group_id) ?>"><img width="10px" height="10px" src="<?php echo yii::$app->request->baseUrl?>/assets/images/laji.jpg" alt="" /></a>
                            </td>
                        </tr>
                    <?php endforeach;?>
                    </tbody>
